...
mySVGOut = my_output_file(
    filename, plot_type="Violin", extension="svg"
)  # regex to generate a new filename based on the .csv filename


# GET COLORS AND DEFINTE AESTHETIC PARAMETERS
sns.set_theme(
    rc={"figure.figsize": (5, 2)}
)  # sets the global parameters for graphing objects
WhiteWheel = ["#FFFFFF"] * len(list(set(DataSet[white_overlay])))

# MAKE THE VIOLIN PLOT
g = sns.violinplot(
    data=DataSet,  # data: data object being plotted
...
ax = sns.swarmplot(
    data=DataSet,  # data object being graphed
...
g.savefig(
    mySVGOut
)  # saves the file to a .svg format with the regex filename from earlier
